...
export default function SessionsClient({
  ...
  useEffect(() => {
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      if (newNext < MIN_PCT) {
        newNext = MIN_PCT;
        newCurrent = startWidths[colIndex] + startWidths[colIndex + 1] - MIN_PCT;
      }
      ...
      <Card className="hidden sm:block">
        <CardContent className="p-0">
          <Table ref={tableRef} style={{ tableLayout: "fixed", width: "100%" }}>
            ...
            <TableHeader>
              <TableRow>
                {["Device", "IP", "Last active", "Expires", "Action"].map((header, i) => (
                  <TableHead
                    key={header}
                  ...
                  >
                    ...
                    {i < 4 && (
                      <div
                        ...
                        onMouseEnter={(e) => {
                          (e.currentTarget as HTMLDivElement).style.background = "hsl(var(--border))";
                        }}
                        onMouseLeave={(e) => {
            ...
            <TableBody>
              {sessions.map((session, idx) => (
                <TableRow key={session.id}>
                  ...
                  <TableCell className="overflow-hidden truncate text-center text-sm text-muted-foreground">
                    {session.ipAddress || "—"}
                  </TableCell>
                  <TableCell className="overflow-hidden truncate text-center text-sm text-muted-foreground">
